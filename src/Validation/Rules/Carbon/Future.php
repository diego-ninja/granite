<?php
// ABOUTME: Defines Future as part of validation rule definition and execution.
// ABOUTME: Owns the Future boundary between rule definitions and validation results.
...
/**
 * Validation rule for Carbon future dates.
 * Validates that a date is in the future.
 */
final class Future extends AbstractRule
{
    /**
     * Constructor.
     *
    ...
    public function validate(mixed $value, ?array $allData = null): bool
    {
        ...
        if ($this->allowToday) {
            if ($now instanceof CarbonInterface) {
                return $value->startOfDay()->greaterThanOrEqualTo($now->startOfDay());
            }
        }

        return $value->greaterThan($now);
    }
    ...
    {
        if ($this->allowToday) {
            return sprintf("%s must be today or in the future", $property);
        }

        return sprintf("%s must be in the future", $property);
    }
}
